...
class Generator:
    def __init__(self, start_res_log2, target_res_log2):
        ...
        self.g_input = tf.keras.layers.Input(self.input_shape, name="generator_input")

        for i in range(start_res_log2, target_res_log2 + 1):
            filter_num = self.filter_nums[i]
            res = 2 ** i
            self.noise_inputs.append(
                tf.keras.layers.Input(shape=(res, res, 1), name=f"noise_{res}x{res}")
            )
            to_rgb = tf.keras.Sequential(
                [
                    tf.keras.layers.InputLayer(input_shape=(res, res, filter_num)),
    ...
    def build_block(self, filter_num, res, input_shape, is_base):
        input_tensor = tf.keras.layers.Input(shape=input_shape, name=f"g_{res}")
        noise = tf.keras.layers.Input(shape=(res, res, 1), name=f"noise_{res}")
        w = tf.keras.layers.Input(shape=512)
        x = input_tensor
    ...
    def grow(self, res_log2):
        ...
        w = tf.keras.layers.Input(shape=(self.num_stages, 512), name="w")

        alpha = tf.keras.layers.Input(shape=(1), name="g_alpha")
...
class Discriminator:
    ...
    def build_base(self, filter_num, res):
        input_tensor = tf.keras.layers.Input(shape=(res, res, filter_num), name=f"d_{res}")
        ...
        return tf.keras.Model(input_tensor, x, name=f"d_{res}")

    def build_block(self, filter_num_1, filter_num_2, res):
        input_tensor = tf.keras.layers.Input(shape=(res, res, filter_num_1), name=f"d_{res}")
    ...
    def grow(self, res_log2):
        ...
        alpha = tf.keras.layers.Input(shape=(1), name="d_alpha")
        input_image = tf.keras.layers.Input(shape=(res, res, 3), name="input_image")
        x = self.from_rgb[idx](input_image)
        x = self.d_blocks[idx](x)
        if idx > 0:
            idx -= 1
            downsized_image = tf.keras.layers.AveragePooling2D((2, 2))(input_image)
            y = self.from_rgb[idx](downsized_image)
            x = fade_in(alpha[0], x, y)

            for i in range(idx, -1, -1):
                x = self.d_blocks[i](x)
        return tf.keras.Model([input_image, alpha], x, name=f"discriminator_{res}_x_{res}")
